...
# template_icon = os.path.join(absolute_path, "src/_TemplateIcon.tsx")

# def to_camel_case(kebab_str):
#     components = kebab_str.split('-')
#     return ''.join(x.capitalize() for x in components)

# def createReactComponent(svg, folder):
#   with open(template_icon, "r") as f:
#     template = f.read()
#     svg_name = svg.replace(".svg", "")
#     svg_name = to_camel_case(svg_name)
#     if not svg_name[0].isnumeric():
#       component = template.replace("TemplateIcon", svg_name).replace("tempico", svg.replace(".svg", ""))
#       new_file = open(os.path.join(absolute_path, relative_icons_path + folder + "/" + svg_name + ".tsx"), "w")
#       new_file.write(component)
#       new_file.close()

# for folder in res:
#   for svg in res[folder]:
#     createReactComponent(svg, folder)
...
for folder in os.listdir(full_icons_path):
  ...
  for path in os.listdir(full_icons_path + "/" + folder):
    if os.path.isfile(os.path.join(full_icons_path + "/" + folder, path)):
      icons[folder].append("export { default as " + path.replace(".tsx", "") + " } from './" + folder + "/" + path.replace(".tsx", "") + "';\n")
      icon_folder = open(os.path.join(absolute_path, "src/icons/" + folder.capitalize() +".ts"), "w")
      for icon in icons[folder]:
        icon_folder.write(icon)
